...
const Home = () => {
  ...
  const renderOledContent = () => {
    if (oledView === 'menu') {
      return (
        <>
          <div className="oled-header">
            <span className="oled-title">EDISON ENRIQUEZ</span>
            <span className="oled-status">READY</span>
          </div>
          
          <div className="oled-menu">
            {menuItems.map((item, index) => (
              <div 
                key={index}
                className={`oled-menu-item ${index === selectedMenu ? 'selected' : ''}`}
              >
                {index === selectedMenu && <span className="menu-cursor">&gt;</span>}
      ...
      const sections = ['personal', 'experience', 'education', 'skills'];
      const sectionNames = ['INFO', 'EXPERIENCE', 'EDUCATION', 'SKILLS'];
      const currentSection = sections[cvSection];
      
      return (
        <>
          <div className="oled-header">
            <span className="oled-title">{sectionNames[cvSection]}</span>
            <span className="oled-status">{cvSection + 1}/{sections.length}</span>
          </div>
          
          <div className="oled-cv-content">
            {currentSection === 'personal' && (
              <div className="cv-section">
                <div className="cv-line">{cvData.personal.name}</div>
                <div className="cv-line cv-muted">{cvData.personal.title}</div>
                <div className="cv-line cv-small">{cvData.personal.email}</div>
                <div className="cv-line cv-small">{cvData.personal.linkedin}</div>
              </div>
            )}
            
            {currentSection === 'experience' && cvData.experience && (
              <div className="cv-section">
                <div className="cv-line cv-bold">{cvData.experience[0].title}</div>
                <div className="cv-line cv-small">{cvData.experience[0].company}</div>
                <div className="cv-line cv-muted cv-small">{cvData.experience[0].period}</div>
                <div className="cv-line cv-small">{cvData.experience[0].responsibilities[0].substring(0, 40)}...</div>
              </div>
            )}
            
            {currentSection === 'education' && cvData.education && (
              <div className="cv-section">
                <div className="cv-line cv-bold">{cvData.education[0].degree}</div>
                <div className="cv-line cv-small">{cvData.education[0].institution}</div>
                <div className="cv-line cv-muted cv-small">{cvData.education[0].period}</div>
              </div>
            )}
            
            {currentSection === 'skills' && cvData.skills && (
              <div className="cv-section">
                {cvData.skills.slice(0, 4).map((skill, idx) => (
                  <div key={idx} className="cv-line cv-small">
                    <span className="cv-bullet">-</span> {skill.name}
                  </div>
                ))}
              </div>
            )}
